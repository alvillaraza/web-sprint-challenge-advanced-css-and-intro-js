...
artists.map((artist) => {
  ...
  console.log("vincent", artists[8].name);
});
...
function get20s(array) {
  ...
  array.map((item) => {
    //split the years, so it can be parsed from a string into a numbers
    const years = item.years.split(" - ");
    // console.log("years", item, years);
    const parsed = years.map((year) => JSON.parse(year));

    if (parsed[0] >= 1900 && parsed[1] <= 2000) {
      console.log(item);
    }
  });
}
...
function removeArtist(array, index) {
  // removes an artist from array at index
  array.splice(index, 1);
  //logs the length of data
...
function lotsOfArt(array) {
  array.map((item) => {
    ...
    if (item.paintings > 100) console.log(item.name, item.paintings);
  });
}
